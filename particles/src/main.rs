...
unsafe impl GlobalAlloc for ReportingAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        let ptr = System.alloc(layout);  // Defers the memory allocation to the system's default allocator
        let end = Instant::now();
...
impl World {
    ...
    fn add_shapes(&mut self, n: i32) {
        for _ in 0..n.abs() {
            let particle = Particle::new(&self); // Creates a particle variable on the stack
            let boxed_particle = Box::new(particle); // Takes ownership of particle and moves data to heap, creating reference of data on the stack
